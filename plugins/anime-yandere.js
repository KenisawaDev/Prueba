...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!text) throw m.reply('Ingresa una consulta / yande.re Url');
  
  let [query, page] = text.split(' ');
  let res = await getYandeImage(query, page);

  if (res === 'in_progress') {
    await conn.sendMessage(m.chat, 'Descargando imagen. Espere por favor...', 'conversation', { quoted: m });
    return;
  }
  ...
  text.match(URL_REGEX)
    ? await conn.sendMessage(
        m.chat,
        { [mime.split('/')[0]]: { url: res }, caption: `Exito al descargar: ${await shortUrl(res)}` },
        { quoted: m }
      )
...
handler.help = handler.alias = ['yandere', 'yande'].map((v) => v + ' <consulta> [page]');
handler.tags = ["anime"];
...
async function getYandeImage(query, page = '') {
  if (query.match(URL_REGEX)) {
    ...
    if (json.length === 0) throw m.reply(`Consulta "${query}" no encontrado :/`);
    let data = json[~~(Math.random() * json.length)];
    if (!data) throw m.reply(`Consulta "${query}" no encontrado :/`);
