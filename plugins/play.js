...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw m.reply(`Primero ingresa un titulo de un vídeo de YouTube\nEjemplo: ${usedPrefix + command} Joji - Ew`);
...
//    conn.sendMessage(m.chat, {image: {url: yt_play[0].thumbnail}, caption: texto1}, {quoted: m});
    if (command == 'play') {
    try {   
    const audio = `https://api.cafirexos.com/api/v2/ytmp3?url=${yt_play[0].url}`
    ...
    if (size >= limit_a2) {  
    await conn.sendMessage(m.chat, {text: `*Descargue su audio aquí ${audio}*`}, {quoted: m});
    return;    
    }     
    ...
    throw m.reply('Ocurrió un error. Por favor, inténtalo de nuevo más tarde.');    
    }}
    if (command == 'play2') {
    try {   
    const video = `https://api.cafirexos.com/api/v2/ytmp4?url=${yt_play[0].url}`
    ...
    if (size2 >= limit2) {  
    await conn.sendMessage(m.chat, {text: `Descargue su vídeo en ${video}*`}, {quoted: m});
    return;    
    }     
